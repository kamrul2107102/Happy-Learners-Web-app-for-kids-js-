       ...
       if (q.type === 'multiple_choice'){
         q.options.forEach(opt=>{
           ...
           optEl.addEventListener('click', ()=>{
             ...
             setTimeout(()=>{ index++; if (index < total) startNext(); else endQuiz() }, 700);
           });
         ...
         function checkDragMatchCompletion(){
           ...
           if (zones.every(z=>z.dataset.got)){
             ...
             setTimeout(()=>{ index++; if (index < total) startNext(); else endQuiz(); }, 900);
           }
         }
         ...
         items.forEach(it=>{
           ...
           itEl.addEventListener('click', ()=>{
             itEl.style.opacity = 0.5; itEl.style.pointerEvents = 'none';
         ...
         submit.addEventListener('click', ()=>{
           const placed = Array.from(list.children).map(x=>x.textContent);
           const correct = q.answerOrder;
           if (JSON.stringify(placed) === JSON.stringify(correct)){ score++; playTone('correct'); }
           else playTone('incorrect');
           setTimeout(()=>{ index++; if (index < total) startNext(); else endQuiz(); }, 700);
         });
       ...
       if (opts.timer){
         ...
         timerInterval = setInterval(()=>{
           ...
           if (timeLeft <= 0){
             ...
             playTone('incorrect');
             index++;
             if (index < total) startNext(); else endQuiz();
           }
         },1000);
       }
     }
